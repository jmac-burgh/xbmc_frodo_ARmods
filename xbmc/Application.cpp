 ...
 *
 *
 *  This file was modified by jmac 03/22/2013 to block scrapped media flag values from being overwritten by DVDPlayer.
 *  Code mods begin at line 4162.
 *  A flag is checked to see if we want to prevent overwrites of video database when a video is played.  It is created by a python add-on script in the skin, it should
 *  enable user to toggle this mod on and off from the add-on if they don't want to stop video database overwrites.
 *     						jmac
 */
...
bool CApplication::PlayFile(const CFileItem& item, bool bRestart)
{
  ...
  if(bResult)
  {
    ...
    if( IsPlayingVideo() )
    {
	  ...
	  // then check to see if flag exists and if so 
	  // bypass writes to video database by player.    jmac 03/22/2013
	  //
